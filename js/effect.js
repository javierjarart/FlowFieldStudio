...
export class Effect {
  ...
  async init() {
    ...
    let RendererClass = Canvas2DRenderer;
    if (navigator.gpu) {
      try {
        const mod = await import('./renderers/webgpu.js');
        RendererClass = mod.WebGPURenderer;
      } catch {
        if (typeof WebGPURenderer !== 'undefined') RendererClass = WebGPURenderer;
      }
    }
    this.renderer = new RendererClass(this);
    try {
      if (RendererClass !== Canvas2DRenderer) await this.renderer.init();
    } catch (e) {
