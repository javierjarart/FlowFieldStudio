...
function bindExport() {
  ...
  exportBtn.addEventListener('click', () => {
    ...
    const code = [
      ...
      '',
      'class TextParticle{constructor(e){this.e=e;this._init()}_init(){var e=this.e,s=S.txt,cl=e.textCells[Math.floor(Math.random()*e.textCells.length)];this.x=cl.x+Math.random()*e.cellSize;this.y=cl.y+Math.random()*e.cellSize;this.speedMod=s.speedMin+Math.random()*(s.speedMax-s.speedMin);this.maxLen=Math.floor(s.trailMin+Math.random()*(s.trailMax-s.trailMin));this.history=[{x:this.x,y:this.y}];this.timer=this.maxLen*2;if(s.colorMode=="image")this.color=cl.color;else if(s.colorMode=="white")this.color="rgba(255,255,255,"+s.opacity+")";else this.color=s.solidColor}draw(c){if(this.history.length<2)return;c.save();c.globalAlpha=S.txt.opacity;c.lineWidth=S.txt.lineWidth;c.strokeStyle=this.color;c.globalCompositeOperation=S.blendMode;c.beginPath();c.moveTo(this.history[0].x,this.history[0].y);for(var i=1;i<this.history.length;i++)c.lineTo(this.history[i].x,this.history[i].y);c.stroke();c.restore()}update(){this.timer--;var e=this.e;if(this.timer>=1){var col=Math.max(0,Math.min(Math.floor(this.x/e.cellSize),e.columns-1)),row=Math.max(0,Math.min(Math.floor(this.y/e.cellSize),e.rows-1)),f=e.flowField[row*e.columns+col],a=f?f.angle:0,bo=(f&&f.inText)?S.txt.boost:1;this.x+=Math.cos(a)*this.speedMod*bo;this.y+=Math.sin(a)*this.speedMod*bo;this.history.push({x:this.x,y:this.y});if(this.history.length>this.maxLen)this.history.shift()}else if(this.history.length>1)this.history.shift();else this._init()}}',
      '',
      'class BgParticle{constructor(e){this.e=e;this._init()}_init(){var e=this.e,s=S.bg,t=0;do{this.x=Math.random()*e.width;this.y=Math.random()*e.height;t++}while(s.avoidText&&e.isTextZone(this.x,this.y)&&t<20);this.speedMod=s.speedMin+Math.random()*(s.speedMax-s.speedMin);this.maxLen=Math.floor(s.trailMin+Math.random()*(s.trailMax-s.trailMin));this.history=[{x:this.x,y:this.y}];this.timer=this.maxLen*2;this.color=s.color}draw(c){if(this.history.length<2)return;c.save();c.globalAlpha=S.bg.opacity;c.lineWidth=S.bg.lineWidth;c.strokeStyle=this.color;c.globalCompositeOperation=S.blendMode;c.beginPath();c.moveTo(this.history[0].x,this.history[0].y);for(var i=1;i<this.history.length;i++)c.lineTo(this.history[i].x,this.history[i].y);c.stroke();c.restore()}update(){this.timer--;var e=this.e,s=S.bg;if(this.timer>=1){var col=Math.max(0,Math.min(Math.floor(this.x/e.cellSize),e.columns-1)),row=Math.max(0,Math.min(Math.floor(this.y/e.cellSize),e.rows-1)),f=e.bgFlowField[row*e.columns+col],a=f?f.angle:0,i=e.isTextZone(this.x,this.y),sp=(s.avoidText&&i)?this.speedMod*0.1:this.speedMod;this.x+=Math.cos(a)*sp;this.y+=Math.sin(a)*sp;this.history.push({x:this.x,y:this.y});if(this.history.length>this.maxLen)this.history.shift();if(this.x<0||this.x>e.width||this.y<0||this.y>e.height)this._init()}else if(this.history.length>1)this.history.shift();else this._init()}}',
      '',
